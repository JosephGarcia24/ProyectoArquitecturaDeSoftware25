...
skinparam class {
  ...
  BackgroundColor<<Laravel>> LightYellow
  BorderColor Black
  ArrowColor Black
}
...
class EmpleadoController <<Controller>> {
  ...
  + show(Empleado $empleado) : View
  + edit(Empleado $empleado) : View
  + update(Request $request, Empleado $empleado) : RedirectResponse
  + destroy(Empleado $empleado) : RedirectResponse
  + destroyMultiple(Request $request) : RedirectResponse
}
...
class Empleado <<Model>> {
  ...
  ' Atributos públicos (propiedades del modelo)
  + $id : int
  + $nombre : string
  + $email : string
  + $departamento : string
  ...
  + {static} create(array $attributes) : Empleado
  + {static} find(int $id) : Empleado|null
  + {static} where(string $column, mixed $value) : Builder
  + {static} whereIn(string $column, array $values) : Builder
  + {static} paginate(int $perPage = 15) : LengthAwarePaginator
  + {static} latest(string $column = "created_at") : Builder
...
class EmpleadoException <<Exception>> {
  ...
  + {static} noEncontrado(int|string $id) : EmpleadoException
  + {static} errorAlCrear(string $mensaje = null) : EmpleadoException
  + {static} errorAlActualizar(string $mensaje = null) : EmpleadoException
  + {static} errorAlEliminar(string $mensaje = null) : EmpleadoException
  + {static} emailDuplicado(string $email) : EmpleadoException
  + {static} ningunEmpleadoSeleccionado() : EmpleadoException
  + {static} errorEliminacionMultiple(string $mensaje = null) : EmpleadoException
}

' Clases de Laravel (Framework)
class Controller <<Laravel>> {
  ' Clase base de Laravel
}
...
class View <<Laravel>> {
  ' Representa una vista Blade
}

class RedirectResponse <<Laravel>> {
  + with(string $key, mixed $value) : RedirectResponse
  + withInput() : RedirectResponse
}

class Builder <<Laravel>> {
  + where(string $column, mixed $value) : Builder
  + whereIn(string $column, array $values) : Builder
  + latest(string $column) : Builder
  + paginate(int $perPage) : LengthAwarePaginator
  + delete() : int
}

class QueryException <<Laravel>> {
  + errorInfo : array
}

' Relaciones de Herencia
EmpleadoController --|> Controller : extends
Empleado --|> Model : extends
EmpleadoException --|> Exception : extends

' Relaciones de Dependencia (uso)
EmpleadoController ..> Request : <<usa>>
EmpleadoController ..> View : <<retorna>>
EmpleadoController ..> RedirectResponse : <<retorna>>
EmpleadoController ..> Empleado : <<usa>>
EmpleadoController ..> EmpleadoException : <<lanza>>
EmpleadoController ..> QueryException : <<captura>>

Empleado ..> Builder : <<retorna>>
Empleado ..> Model : <<usa métodos>>

' Asociación entre Controlador y Modelo
EmpleadoController --> Empleado : gestiona

' Asociación entre Controlador y Exception
EmpleadoController --> EmpleadoException : lanza excepciones

' Notas explicativas
note right of EmpleadoController
  Controlador RESTful que implementa
  todas las operaciones CRUD y
  gestiona la lógica de negocio
end note

note right of Empleado
  Modelo Active Record con
  Eloquent ORM. Implementa
  mass assignment protection
end note

note bottom of EmpleadoException
  Excepciones personalizadas
  con métodos factory para
...
legend right
  |<back:LightBlue> Controller </back>| Capa de Control |
  |<back:LightGreen> Model </back>| Capa de Datos |
  |<back:LightPink> Exception </back>| Manejo de Errores |
  |<back:LightYellow> Laravel </back>| Framework |
endlegend
